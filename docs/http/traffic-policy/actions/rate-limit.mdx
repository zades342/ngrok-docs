...
| `bucket_key` | Set&lt;string&gt; | The elements of this collection define the unique key of a request to collect and track the rate at which the capacity is being met. Possible values are `"req.Host"`, which is the Host, `"conn.ClientIP"`, and `"req.Headers['x-example-header-name']"` or the related macro `getReqHeader('x-example-header-name')`, which is the value for the specified header key, if it exists. Up to ten bucket keys can be specified. |
