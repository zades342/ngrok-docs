...
This rule denies all HTTP traffic that is not a GET request.
...
We deliver tailored content to Microsoft Edge users by examining the `User-Agent` header for the case-insensitive string `(?i)edg/` succeeded by digits `\d`. To see how this works in practice, explore the following [regex101 demonstration](https://regex101.com/r/3NPVub/1).
